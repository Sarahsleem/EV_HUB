...
class EditCarCubit extends Cubit<EditCarState> {
  EditCarCubit(this.editCarRepo) : super(EditCarInitial());
  EditCarRepo editCarRepo;
...
void setChargeType(String type) {
  ...
  Future<void> pickMultipleImages() async {
    ...
    for (final file in pickedFiles) {
      ...
      emit(LoadingCarImages());

      try {
        final response = await editCarRepo.postCarImage(newImage);
  ...
  Future<void> pickFeaturedImage() async {
    ...
    emit(LoadingCarImage());
    if (pickedFile != null) {
      try {
        image = File(pickedFile.path);
        final response = await editCarRepo.postCarImage(image!);
  ...
  Future<void> updateCar(String title, String describe,String price,String enpower,String battery,String km,int carId) async {
    ...
    emit(EditCarLoading());
    var response = await editCarRepo.editCars(carId,carModel);
    response.fold(
          (l) {
        emit(EditCarFailare());
      },
