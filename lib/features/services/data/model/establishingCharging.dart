class EstablishingCharging {
  ...
  Map<String, dynamic> toMap() {
    return {
      ...
      'location_map': this.locationMap,
      'featured_media_url': this.featuredMediaUrl,
    };
  }

  factory EstablishingCharging.fromMap(Map<String, dynamic> map) {
    return EstablishingCharging(
      id: map['id'] ?? 0,
      date: map['date'] ?? '',
      slug: map['slug'] ?? '',
      ...
      locationMap: map['acf']?['location_map'] ?? '',
      featuredMediaUrl: map['_embedded']?['wp:featuredmedia']?[0]?['source_url'], // Extract the featured media URL
    );
  }
}